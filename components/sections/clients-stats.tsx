...
function StatCounter({ value, suffix = "", label, description }: StatItemProps) {
  ...
  return (
    <div className="flex flex-col p-8 bg-ump-alt/60 rounded-3xl border border-white/5 backdrop-blur-sm group hover:border-ump-accent/30 transition-colors duration-500">
      <div className="text-5xl md:text-6xl font-black text-white mb-2 flex items-baseline tracking-tight">
        <span ref={ref}>0</span>
...
export function ClientsStats() {
  ...
  return (
    <section
      ref={containerRef}
      className="py-24 md:py-32 bg-ump-background relative overflow-hidden rounded-t-[2.5rem] md:rounded-t-[5rem] -mt-8 md:-mt-12 z-20 border-t border-white/5 shadow-[0_-5px_20px_rgba(0,0,0,0.2)]"
    >
      <div className="container mx-auto px-6 max-w-7xl">
        ...
        {/* Clients Marquee */}
        <div className="relative w-full overflow-hidden py-10 border-y border-white/5 bg-ump-alt/20">
          ...
          <div className="flex select-none overflow-hidden gap-12 whitespace-nowrap">
            ...
            >
              {[...CLIENTS, ...CLIENTS].map((client, idx) => (
                <span
                  key={idx}
                  className="text-2xl md:text-3xl font-black text-white/30 tracking-tight uppercase hover:text-ump-accent transition-colors duration-300 pointer-events-auto cursor-default font-mono"
                >
                  // {client}
                </span>
              ))}
